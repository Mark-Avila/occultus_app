...
class StreamPage(ctk.CTkToplevel):
    ...
    def on_privacy_select(self, value: str):
        self.occultus.change_privacy_type(value.lower())
    ...
    def on_censor_select(self, value: str):
        ...
        if keys[value] == "default":
            self.occultus.change_censor_type(keys[value])
        else:
            self.occultus.change_censor_type(keys[value])
    ...
    def on_feed_click(self, event):
        ...
        if self.dets is not None:
            for det in self.dets:
                bboxes = det[1:]
                tracking_id = int(det[0])
                if self.is_point_inside_box(coords, bboxes):
                    if tracking_id not in self.occultus.get_ids():
                        self.occultus.add_id(int(tracking_id))
                        self.id_list_frame.add_item(tracking_id)
    ...
    def webcam_thread(self):
        ...
        self.occultus = Occultus(
            conf_thres=0.2, censor_type="detect", privacy_type="all"
        )
        self.occultus.change_censor_type("detect")
        self.dets = []
        imgtk = None
        ...
        while True:
            ...
            og_img, bboxes = self.occultus.detect(in_frame)
